...
\begin{center}
\texttt{[Repository URL to be provided]}
\end{center}
...
The complete Lean 4 proof is available at:
\begin{center}
\url{https://github.com/machinelearning2014/apery_theorem/blob/cd308d6f1021815e36a0dabbc193285b8077a6d3/proof_v2.lean}
\end{center}

This appendix includes the full source code for reference. The code is extensively commented and demonstrates the complete proof of Ap\'ery's theorem in Lean 4 with Mathlib.
...
-- Full proof continues with all theorems and lemmas
-- See the GitHub repository for the complete code
-- (Code truncated for space - see online version)

end
\end{lstlisting}

\noindent\textbf{Note:} The complete source code (over 500 lines) is too lengthy to include in full here. Please refer to the GitHub repository for the complete, compilable proof.

\subsection{Key Components}

The formalization includes:
\begin{itemize}[noitemsep]
\item \textbf{Lines 1-22}: Imports and setup
\item \textbf{Lines 23-65}: Integral representation of $\zeta(3)$
\item \textbf{Lines 66-120}: Legendre polynomial bounds
\item \textbf{Lines 121-180}: Function $\varphi$ and polynomial bounds for $I_n$
\item \textbf{Lines 181-280}: Explicit computation of $I_1$ 
\item \textbf{Lines 281-380}: Recurrence relation for $I_n$
\item \textbf{Lines 381-450}: Beukers representation existence
\item \textbf{Lines 451-500}: Exponential growth of denominators
\item \textbf{Lines 501-560}: Approximation conditions
\item \textbf{Lines 561-580}: Final irrationality proof via Liouville's theorem
\end{itemize}
